...
type PortGroupLister interface {
	...
	List(selector labels.Selector) (ret []*multiovnv1.PortGroup, err error)
...
type portGroupLister struct {
	listers.ResourceIndexer[*multiovnv1.PortGroup]
}

// NewPortGroupLister returns a new PortGroupLister.
func NewPortGroupLister(indexer cache.Indexer) PortGroupLister {
	return &portGroupLister{listers.New[*multiovnv1.PortGroup](indexer, multiovnv1.Resource("portgroup"))}
}

// PortGroups returns an object that can list and get PortGroups.
func (s *portGroupLister) PortGroups(namespace string) PortGroupNamespaceLister {
	return portGroupNamespaceLister{listers.NewNamespaced[*multiovnv1.PortGroup](s.ResourceIndexer, namespace)}
}
...
type PortGroupNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*multiovnv1.PortGroup, error)
